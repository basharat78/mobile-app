...
//     }
    ...
    public function saveLoad()
    {
        $this->validate([
            ...
            'deadhead' => 'required|integer|min:0',
            'total_miles' => 'required|integer|min:0',
    ...
    {
        $this->editingLoadId = null;
        $this->reset(['pickup_location', 'drop_location', 'miles', 'rate', 'notes', 'equipment_type', 'pickup_time', 'drop_off_time', 'deadhead', 'total_miles', 'rpm', 'weight', 'broker_name']);
    }
        ...
        <div class="bg-slate-800/50 border border-white/5 rounded-[2.5rem] p-8 space-y-6 lg:sticky lg:top-8">
            ...
            <form wire:submit="saveLoad" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Pick Location</label>
                        <input wire:model="pickup_location" type="text" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="City, ST">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Pick Time</label>
                        <input wire:model="pickup_time" type="text" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="08:00 AM">
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Drop Off Location</label>
                        <input wire:model="drop_location" type="text" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="City, ST">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Drop Off Time</label>
                        <input wire:model="drop_off_time" type="text" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="4:00 PM">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Miles</label>
                        <input wire:model.live="miles" type="number" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="500">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Deadhead</label>
                        <input wire:model.live="deadhead" type="number" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="50">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Total Miles</label>
                        <input wire:model="total_miles" type="number" readonly class="w-full bg-slate-800 border border-white/5 rounded-2xl px-5 py-4 text-slate-400 text-sm outline-none font-bold cursor-not-allowed">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Rate ($)</label>
                        <input wire:model.live="rate" type="number" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="1500">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">RPM</label>
                        <input wire:model="rpm" type="text" readonly class="w-full bg-slate-800 border border-white/5 rounded-2xl px-5 py-4 text-slate-400 text-sm outline-none font-bold cursor-not-allowed">
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Weight (lbs)</label>
                        <input wire:model="weight" type="number" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="45000">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Equipment Type</label>
                        <select wire:model="equipment_type" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Target Carrier</label>
                        <select wire:model="carrier_id" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold">
                            <option value="">Select Carrier</option>
                            @foreach($this->carriers as $carrier)
                                <option value="{{ $carrier->id }}">{{ $carrier->user->name ?? 'Unknown' }} ({{ $carrier->user->company_name ?? 'No Company' }})</option>
                            @endforeach
                        </select>
                        @error('carrier_id') <span class="text-red-500 text-[10px] font-bold ml-2">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Broker Name</label>
                        <input wire:model="broker_name" type="text" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="Brokerage Inc.">
                    </div>
                </div>

                <div>
                    <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2 mb-1">Note</label>
                    <textarea wire:model="notes" rows="2" class="w-full bg-slate-900 border border-white/5 rounded-2xl px-5 py-4 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="Additional details..."></textarea>
                </div>
                ...
                @foreach($this->loads as $load)
                    <div class="bg-slate-800/30 border {{ $editingLoadId == $load->id ? 'border-blue-500/50' : 'border-white/5' }} rounded-[2.5rem] p-6 group hover:border-blue-500/30 transition-all">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 bg-blue-600/10 rounded-2xl flex items-center justify-center font-black text-blue-500 italic text-xl shrink-0">
                                    {{ substr($load->equipment_type ?? 'X', 0, 1) }}
                                </div>
                                <div class="flex-1">
                                    <div class="flex flex-wrap items-center gap-2 mb-1">
                                        <div class="flex items-center gap-2 px-2 py-1 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3 h-3 text-blue-400">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                                            </svg>
                                            <span class="text-[9px] font-black text-blue-400 uppercase tracking-widest">For: {{ $load->carrier->user->name ?? 'Unknown' }}</span>
                                        </div>
                                    </div>
                                    <div class="flex flex-wrap items-center gap-2 mb-1">
                                        <h4 class="text-white font-bold text-lg">{{ $load->pickup_location }}</h4>
                                        @if($load->pickup_time)
                                            <span class="px-2 py-0.5 bg-blue-500/10 border border-blue-500/20 rounded text-[9px] font-black text-blue-400 uppercase tracking-widest shrink-0">{{ $load->pickup_time }}</span>
                                        @endif
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 text-slate-600 shrink-0">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                                        </svg>
                                        <h4 class="text-white font-bold text-lg">{{ $load->drop_location }}</h4>
                                        @if($load->drop_off_time)
                                            <span class="px-2 py-0.5 bg-blue-500/10 border border-blue-500/20 rounded text-[9px] font-black text-blue-400 uppercase tracking-widest shrink-0">{{ $load->drop_off_time }}</span>
                                        @endif
                                    </div>
                                    <div class="flex flex-wrap gap-x-4 gap-y-1">
                                        <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest whitespace-nowrap">{{ $load->miles }} Miles @if($load->deadhead) (+{{ $load->deadhead }} DH) @endif • {{ $load->equipment_type }}</p>
                                        @if($load->weight)
                                            <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest whitespace-nowrap">{{ number_format($load->weight) }} lbs</p>
                                        @endif
                                        @if($load->rpm > 0)
                                            <p class="text-[10px] text-blue-500 font-black uppercase tracking-widest whitespace-nowrap">${{ $load->rpm }}/mi</p>
                                        @endif
                                        @if($load->broker_name)
                            ...
                            <div class="text-left md:text-right ml-16 md:ml-0 mt-2 md:mt-0">
                                <span class="text-2xl font-black text-white italic tracking-tighter block leading-none">${{ number_format($load->rate) }}</span>
                        ...
                        @if($load->requests->count() > 0)
                            <div class="mt-4 pt-4 border-t border-white/5 space-y-3">
                                <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Bids / Requests ({{ $load->requests->count() }})</p>
                                @foreach($load->requests as $request)
                                    <div class="bg-slate-900/50 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group/bid hover:bg-slate-900 transition-colors">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center text-xs font-bold text-slate-400 group-hover/bid:bg-blue-600 group-hover/bid:text-white transition-all">
                                                {{ substr($request->carrier->user->name ?? '?', 0, 1) }}
                                            </div>
                                        ...
                                        <div class="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                                            <span class="px-3 py-1 bg-{{ $request->status === 'approved' ? 'green' : ($request->status === 'pending' ? 'yellow' : 'red') }}-500/10 text-{{ $request->status === 'approved' ? 'green' : ($request->status === 'pending' ? 'yellow' : 'red') }}-500 rounded-full text-[10px] font-black uppercase tracking-widest shrink-0">
                                                {{ $request->status }}
                                            </span>
                                            @if($request->status === 'pending' && $load->status === 'available')
                                                <button wire:click="approveBid({{ $request->id }})" class="flex-1 sm:flex-none px-4 py-2 bg-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-blue-500/20 hover:bg-blue-500 transition-all active:scale-95">Approve</button>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
